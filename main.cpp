...
void SetConsoleColor(int);//设置控制台颜色
void shopset(Linchong &linchong);//进入商店
void runDevelopmentSystem();//养成系统
void pre_battle1(vector<Character*>& enemies, Linchong* lc, Character* character1, Character* character2,Character* character3);//三个敌人的战斗
void pre_battle2(vector<Character*>&enemies, Linchong* lc, Character* character1, Character* character2);//两个敌人的战斗
void change1();//幕与幕之间衔接
void ready_save();//储存预备
int change2();//显示菜单

void we_save();//保存武器到本地
void we_read();//读取武器装备
void ar_save();//保存护甲到本地
void ar_read();//读取护甲
void os_save(offensiveSkill[6]);//保存攻击技能
void os_read();//读取攻击技能
void su_save(supportSkill[11]);//保存功能技能
void su_read();//读取功能技能
...
int main(){
    ...
    ui.ready();//显示准备游戏界面，给用户设置控制台大小，输出注意事项
    ...
    while(t.plottime<=10)
    {
        ...
        case 6:
            ...
            map1.showmap(lc->getPlace());//展示地图
            lc->changePlace(t.plottime);//选择所需去的地方，用户交互，此处仅可去市井
            change1(); 
            break;
        case 7:
            plot.PrintScene(t.plottime);//第七幕
            pre_battle2(enemies, lc, smallen1, smallen2);//与两个土匪战斗
            plot.PrintScene11();//与土匪战斗后的剧情
            lc->setHP(lc->getMaxHP());//回满生命
            change1(); 
            break;
        case 8:
        ...
        case 9:
            ...
            pre_battle1(enemies, lc, bigen1, bigen2, bigen3);//与三个敌人战斗
            change1(); break;
        case 10:
            plot.PrintScene(t.plottime);//第十幕，游戏结束
            ++t.plottime;
            break;
        default:
            break;
